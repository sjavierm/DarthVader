...
public class GameBoard {
	...
	public void generarCaminos() {
		...
		PowerUp moneda = new PowerUp("moneda", 0, 10, 0);
		PowerUp hongo = new PowerUp("hongo", 0, 0, 1);
		PowerUp estrella = new PowerUp("estrella", 1, 0, 0);
		...
		for(int i=0 ; i<casillero[0].length ; i++){
			for(int j=0 ; j<casillero[0].length ; j++)
			{
				int opcion = (int) Math.random() * 1;
				if(opcion == 1)
				{
					int powerup = (int) (Math.random() * 3)+1;
					switch(powerup)
					{
						case 1: casillero[i][j].setPowerUp(moneda);
						break;
						case 2: casillero[i][j].setPowerUp(hongo);
						break;
						case 3: casillero[i][j].setPowerUp(estrella);
					}
				}
			}
		}
